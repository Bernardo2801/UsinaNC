...
@section('title', 'Usina (Editar Solicitação)')
            ...
            <div class="rounded-lg shadow-lg bg-gray-800 py-10 md:py-12 px-4 md:px-6">
                ...
                <div class="mb-2">
                    <h3 class="px-3 py-1.5 font-semibold text-sm text-white uppercase tracking-widest">Preencha os campos
                        para editar a solicitação.</h3>
                </div>
                <form method="POST" action="{{ route('solicitations.update', $solicitation->id) }}"
                        ...
                        <select name="requester_id" id="requester_id"
                            ...
                            @foreach ($requesters as $requester)
                                <option class="dark:bg-gray-600" value="{{ $requester->id }}"
                                    {{ $solicitation->requester_id == $requester->id ? 'selected' : '' }}>
                        ...
                        <label for="title" class="block text-sm font-medium text-gray-300 uppercase">Selecione o
                            ...
                            <select name="material_id" id="material_id"
                                ...
                                @foreach ($materials as $material)
                                    <option class="dark:bg-gray-600" value="{{ $material->id }}"
                                        {{ $solicitation->material_id == $material->id ? 'selected' : '' }}>
                                        {{ $material->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group mb-6">
                        ...
                        <input type="number" step="0.01"
                            ...
                            value="{{ old('unit_price', $solicitation->unit_price) }}"
                            required>
                    </div>
                    <div class="form-group mb-6" x-data>
                        ...
                        <input type="text" maxlength="22"
                            ...
                            value="{{ old('sei_number', $solicitation->sei_number) }}"
                            required>
                    </div>
                    <div class="form-group mb-6">
                        ...
                        <input type="number" step="0.01" min="0.01"
                            ...
                            value="{{ old('requested_quantity', $solicitation->requested_quantity) }}"
                            required>
                    </div>
                    <div class="form-group mb-6">
                        ...
                        <input type="date" step="0.01" min="0.01"
                            ...
                            value="{{ old('date_solicitation', $solicitation->date_solicitation) }}"
                            required>
                    </div>
                    <div class="form-group mb-6">
                        ...
                        <div class="flex flex-wrap">
                            <div class="flex items-center me-4">
                                <input id="yellow-radio" type="radio" value="Aguardando" name="status"
                    ...
                    <div class="form-group mb-6">
                        ...
                        <select name="driver_id" id="driver_id"
                            ...
                            @foreach ($drivers as $driver)
                                <option class="dark:bg-gray-600" value="{{ $driver->id }}"
                                    {{ $solicitation->driver_id == $driver->id ? 'selected' : '' }}>
                                    {{ $driver->name }}</option>
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="vehicle_id" id="vehicle_id"
                            ...
                            @foreach ($vehicles as $vehicle)
                                <option class="dark:bg-gray-600" value="{{ $vehicle->id }}"
                                    {{ $solicitation->vehicle_id == $vehicle->id ? 'selected' : '' }}>
                    ...
                    <textarea id="message" name="observation" rows="4" maxlength="320"
                        ...
                        placeholder="Caso exista alguma observação sobre esta solicitação, escreva aqui...">{{ old('observation', $requester->observation) }}</textarea>
